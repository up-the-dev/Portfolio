  ...
  const timeline = [
    {
      ...
      description: 'Developing scalable web applications using modern technologies including Golang, Node.js, and React. Leading backend architecture design and implementing secure REST APIs.',
      achievements: [
  ...
  return (
    <section id="experience" className="py-20 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              >
                ...
                <div className={`w-full md:w-5/12 ${index % 2 === 0 ? 'md:text-right md:pr-8' : 'md:text-left md:pl-8'}`}>
                  <motion.div
                    ...
                    className={`bg-white/5 backdrop-blur-sm rounded-2xl p-6 border border-white/10 hover:border-${item.type === 'work' ? 'blue' : 'purple'}-500/50 transition-all duration-300 hover:shadow-2xl hover:shadow-${item.type === 'work' ? 'blue' : 'purple'}-500/25`}
                  >
                    <div className="flex items-center space-x-2 mb-3">
                      <div className={`p-2 rounded-full ${item.type === 'work' ? 'bg-blue-500/20 text-blue-400' : 'bg-purple-500/20 text-purple-400'}`}>
                        <item.icon size={20} />
                      </div>
                      <span className={`px-3 py-1 rounded-full text-sm font-semibold ${item.type === 'work' ? 'bg-blue-500/20 text-blue-300' : 'bg-purple-500/20 text-purple-300'}`}>
                    ...
                    <div className="space-y-2">
                      {item.achievements.map((achievement, achIndex) => (
                        <div key={achIndex} className="flex items-center space-x-2">
                          <div className={`w-2 h-2 rounded-full ${item.type === 'work' ? 'bg-blue-400' : 'bg-purple-400'}`} />
                ...
                <div className="relative z-10 hidden md:block">
                  <motion.div
                    ...
                    className={`w-4 h-4 rounded-full ${item.type === 'work' ? 'bg-blue-500' : 'bg-purple-500'} border-4 border-slate-900 shadow-lg`}
                  />
                </div>
        ...
        >
          <h3 className="text-2xl font-bold text-white mb-8">Recent Recognition</h3>
          <motion.div
            whileHover={{ scale: 1.05 }}
          ...
          >
            <div className="flex items-center justify-center space-x-3">
              ...
              <div>
                <p className="text-xl font-bold text-white">Hidden Gem Annual Award</p>
